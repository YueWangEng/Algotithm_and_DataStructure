...
`sorted(iterable[,key, reverse])`, 系统函数，可用于所有可迭代对象，包括字典（只比较值）和字符串。返回值为一个新的list，无论迭代对象为哪种类型。  
`max(iterable[, key])`, `min(iterable[, key])`，系统函数，可用于所有可迭代对象，包括字典（只比较值）和字符串。返回值为最值。  
