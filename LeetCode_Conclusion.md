...
法2: 哈希表。暴力算法的遍历存在重复，采用哈希算法。可将解题思路改为**向前找匹配值**，已经遍历的部分存入哈希表中，时间复杂度为(n)。  
*对于字典，`in`和`not in`表达式是针对键而非值。*
...
法2: 哈希集合，zip函数(迭代序列元素打包）。另外，一一对照的情况下，可采用zip函数简化此流程，再结合set类型的不可重复性来去掉重复项。
...
法1: 测试函数法。将链表转化为列表再转化为数字，相加，然后将结果按顺序转化为链表。  
这里需要四个函数，链表转列表，列表转数字，数字转列表，列表转链表。  
其中，在列表与数字互转的过程中，可用str辅助法（首选），或者商余函数法，但需要注意0元素造成的影响，例如str法可能导致非零数字的首位为0，商余函数会导致忽略本身为0这种情况。

法2: 设置虚拟头节点，将之后节点val值按位相加。该方法实际使用尾节点法生成目标链表。
两点需要注意：1) 存在进位，可设置辅助值car；2) 两个链表的长度可能不相等，即：一个链表的节点有值，但另一个链表对应的节点已经没值了。

### 21 链表问题，链表合并  

两个链表根据值排序合并，需熟悉链表的生成（头节点法（头节点移动，无尾节点），尾节点法（头节点不动，尾节点移动）），要特别注意顺序，如头节点法对于列表为逆序遍历。  
...
法1: 暴力算法，链表转列表，再转链表（可将列表逆序，也可用头节点法逆序生成链表）

法2: 迭代法，设置辅助节点。每向后移动一位，则反转一位。当一个节点指向一个新的节点以后，原来的指向会自动消失，容易出现的问题：  
1）环状，两个节点相互指向成环；2）节点丢失，未能对节点提前赋值，所以要在将链表分为两端之前提前赋值。 
